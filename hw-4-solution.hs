...
elemList a (Entry b bs) = a == b || elemList a bs

appendList :: List a -> List a -> List a
appendList Empty b = b
appendList a Empty = a
appendList (Entry a Empty) bs = (Entry a bs)
appendList (Entry a as) bs = (Entry a (appendList as bs))

listLength :: List a -> Integer
listLength Empty = 0
listLength (Entry a as) = 1 + listLength as

filterList :: (a -> Bool) -> List a -> List a
filterList _ Empty = Empty
filterList f (Entry a as)
  | f a       = (Entry a (filterList f as))
  | otherwise = filterList f as

nth :: List a -> Integer -> a
nth Empty _ = error "index is too large"
nth (Entry a _) 1 = a
nth (Entry _ as) i = nth as (i-1)

nthMaze :: Integer -> Coord -> Tile
nthMaze n = case nth mazes n of
  (Maze _ f) -> f
...
isGraphClosed initial adjacent isOk = go Empty (Entry initial Empty)
  where
    go _ Empty = True
    go seen (Entry cur res)
      | not (isOk cur)  = False
      | otherwise       = go (Entry cur seen) (filterList (\el -> not (elemList el seen)) (appendList res (adjacent cur)))
...
isClosed :: Maze -> Bool
isClosed (Maze initial f)
  | initialTile /= Ground && initialTile /= Storage  = False
  | otherwise    = isGraphClosed initial adjacent isOk
  where initialTile = f initial
        adjacent c = filterList (\el -> f el /= Wall) (mapList (\d -> adjacentCoord d c) allDirections)
        isOk c
          | f c == Blank = False
          | otherwise    = True
...
noBoxMaze f c = case f c of
  Box -> Ground
  t   -> t

mazeWithBoxes :: (Coord -> Tile) -> List Coord -> Coord -> Tile
mazeWithBoxes f Empty c'        = noBoxMaze f c'
mazeWithBoxes f (Entry c cs) c'
  | eqCoord c c' = Box
  | otherwise  = mazeWithBoxes f cs c'
...
loadLevel n = State c R (initialBoxes f) n
  where (Maze c f) = nth mazes n
initialState :: State
...
tryGoTo (State from _ bx level) d
  ...
  where to          = adjacentCoord d from
        ...
        currentMaze = mazeWithBoxes (nthMaze level) bx
        movedState  = State to d movedBx level
        movedBx     = mapList (moveFromTo to beyond) bx
        didn'tMove  = State from d bx level -- Yes, ' may be part of an identifier

handleEvent :: Event -> State -> State
handleEvent (KeyPress key) s@(State c d bx level)
    | finalLevel && won = (State c d bx level)
    | key == " " && won = loadLevel (level+1)
    | won               = s
    where finalLevel = level == listLength mazes
          won        = gameWon (nthMaze level) bx
...
visualizeMaze :: Maze -> Picture
visualizeMaze (Maze initial f) = colored red (atCoord initial (solidCircle 0.2)) & draw21times (\r -> draw21times (\c -> atCoord (C r c) (drawTile . f $ (C r c))))
...
pictureOfMaze f = draw21times (\r -> draw21times (\c -> drawTileAt f (C r c)))
...
drawTileAt f c = atCoord c (drawTile (noBoxMaze f c))
...
showWin :: (Coord -> Tile) -> List Coord -> Picture
showWin maze cs | gameWon maze cs = scaled 2 2 (text "You won!")
                | otherwise  = blank
...
drawState (State c d boxes level)
  = if level == listLength mazes && gameWon (nthMaze level) boxes then (text "All Complete!") else showWin (nthMaze level) boxes
  & atCoord c (player d)
  & pictureOfBoxes boxes
  & pictureOfMaze (nthMaze level)
...
main :: IO ()
--main = drawingOf . visualizeMaze $ nth extraMazes 1
main = exercise4
